...

{
  ...
  if (s.phase === 'ended' && s.winner !== null) {
    ...
    /*
      «آخر واقف» شرطُ فوز الإسقاط وحده. أمّا الفوز بالوحش الأعظم أو بإفراغ
      اليد فينهي المباراة والثلاثة أحياء، وهما شرطان قائمان لا عيبان.

      وكان هذا الفحص يمرّ لأن الوحش الأعظم كان نادراً: ثماني قطع في سطحٍ
      مشترك من 272 كارتاً. ولمّا صار لكل لاعب ديكه وفيه قطعةٌ من كل نوع
      صار يُستدعى فعلاً — وهو المقصود من الوصفة لا أثرٌ جانبي لها.
    */
    const living = s.players.filter((p) => !p.eliminated && p.hp > 0);
    if (s.winReason?.key === 'reason_hp') {
      if (living.length <= 1) ok('فوز بالإسقاط: لا يبقى أكثر من واقف واحد');
      else fail(`فوز بالإسقاط وقد بقي ${living.length} أحياء`);
    } else {
      ok(`فوز بلا إسقاط (${s.winReason?.key}) — بقاء الثلاثة صحيح`);
    }
